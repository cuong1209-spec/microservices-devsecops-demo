...
Line:        12
Commit:      5c0733b0a259ac386981b0f604fd78efc86c83bf
\end{lstlisting}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{gitleaks_results.png}
    \caption{Log quét bảo mật và phát hiện Secret của Gitleaks trên GitHub Actions}
    \label{fig:gitleaks-results}
\end{figure}

\subsection{Kết quả Semgrep (SAST)}

Semgrep phát hiện lỗi Command Injection trong endpoint tìm kiếm của Product Service:
\begin{lstlisting}[language=bash, caption={Kết quả quét SAST thực tế của Semgrep trên GitHub Actions}]
Scan Summary:
  Findings: 17 (17 blocking)
  Rules run: 495  |  Targets scanned: 18

Selected Blocking Findings:
  demo-app/order-service/app.py
    >> generic.secrets.security.detected-stripe-api-key [BLOCKING]
       Stripe API Key detected
       12| STRIPE_API_KEY = "sk_live_51Habc..."

    >> python.lang.security.insecure-transport [BLOCKING]
       HTTP request detected - use HTTPS instead
       36| response = requests.get(product_url, timeout=5)

    >> python.flask.security.avoid_app_run_with_bad_host [BLOCKING]
       Flask running with host 0.0.0.0 exposes app publicly
       52| app.run(host='0.0.0.0', port=PORT)

  demo-app/order-service/Dockerfile
    >> dockerfile.security.missing-user [BLOCKING]
       Container may run as root - no USER specified

  demo-app/product-service/server.js
    >> javascript.express.security.express-check-csrf-middleware [BLOCKING]
       No CSRF middleware detected in Express application
        3| const app = express();
...
\begin{lstlisting}[language=bash, caption={Kết quả quét SCA thực tế của Trivy trên GitHub Actions}]
Report Summary:
- Target: demo-app/order-service/requirements.txt (Type: pip)
  Vulnerabilities: 1 | Secrets: 0
- Target: demo-app/order-service/app.py (Type: text)
  Vulnerabilities: 0 | Secrets: 1

Findings:
demo-app/order-service/requirements.txt (pip)
=============================================
Total: 1 (HIGH: 1, CRITICAL: 0)
- Library:   werkzeug (Severity: HIGH)
- Vuln ID:   CVE-2024-34069
- Installed: 2.2.3 -> Fixed Version: 3.0.3
- Title:     User may execute code on developer's machine

demo-app/order-service/app.py (secrets)
=======================================
Total: 1 (HIGH: 0, CRITICAL: 1)
- Type:      CRITICAL (stripe-secret-token)
- Details:   Stripe Secret Key detected at line 12
  12| STRIPE_API_KEY = "****************************"
\end{lstlisting}
