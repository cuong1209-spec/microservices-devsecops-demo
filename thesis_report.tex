...
\begin{figure}[H]
    ...
    \fbox{
        \parbox{\textwidth}{
            \textit{[Hình ảnh sơ đồ kiến trúc Pipeline: Developer → Git Push → GitHub Actions → (Song song: Gitleaks, Semgrep, Trivy SCA) → Build Docker Image → Trivy Container Scan → Deploy]}
        }
    }
...
\begin{figure}[H]
    ...
    \fbox{
        \parbox{0.9\textwidth}{
            \textit{[Biểu đồ Gantt so sánh: Thực thi tuần tự (tổng 14 phút) vs. Thực thi song song (tổng 6 phút). Các bar song song gồm: Secret Scan (1 phút), SAST Scan (3 phút), SCA Scan (4 phút - quyết định thời gian tổng) chạy cùng lúc, sau đó mới đến Container Scan (2 phút) tuần tự.]}
        }
    }
